tag @s add echoes_disc_mineral_scar_s2
scoreboard players add @s e_d30 1
advancement grant @s only echoes:root
tellraw @s [{"text":"[Discovery] ","color":"green"},{"text":"Mineral Scar — stage 2/3","color":"gray"}]
execute if score @s e_d30 matches 3 run advancement grant @s only echoes:three_faces_of_time
