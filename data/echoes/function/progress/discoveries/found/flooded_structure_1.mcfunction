tag @s add echoes_disc_flooded_structure_s1
scoreboard players add @s e_d29 1
advancement grant @s only echoes:root
tellraw @s [{"text":"[Discovery] ","color":"green"},{"text":"Flooded Structure — stage 1/3","color":"gray"}]
execute if score @s e_d29 matches 3 run advancement grant @s only echoes:three_faces_of_time
