tag @s add echoes_disc_giant_crater_s3
scoreboard players add @s e_d23 1
advancement grant @s only echoes:root
tellraw @s [{"text":"[Discovery] ","color":"green"},{"text":"Giant Crater — stage 3/3","color":"gray"}]
execute if score @s e_d23 matches 3 run advancement grant @s only echoes:three_faces_of_time
