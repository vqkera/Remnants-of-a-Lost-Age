tag @s add echoes_disc_burned_village_s1
scoreboard players add @s e_d32 1
advancement grant @s only echoes:root
tellraw @s [{"text":"[Discovery] ","color":"green"},{"text":"Burned Village — stage 1/3","color":"gray"}]
execute if score @s e_d32 matches 3 run advancement grant @s only echoes:three_faces_of_time
